...
class LiqPay
{
    ...
    {
        $this->_public_key  = env('LIQPAY_PUPLIC_KEY');
    ...
    public function api(string $path, array $params = [], int $timeout = 5)
    {
        ...
        $data        = $this->encode_params(
            array_merge(compact('public_key'), $params)
        );
    ...
    public function cnb_form($params)
    {
        ...
        return sprintf(
            ...
            $this->_checkout_url,
            sprintf(
                '<input type="hidden" name="%s" value="%s" />', 'data', $data
            ),
            sprintf(
                '<input type="hidden" name="%s" value="%s" />', 'signature',
                $signature
            ),
            $language
        );
