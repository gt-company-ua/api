...
use App\Http\Requests\Admin\GreenCardPriceRequest;
...
class GreenCardController extends Controller
{
        ...
        foreach (Order::INSURANCE_COMPANIES as $company) {
            foreach (GreencardCashback::TRANSPORT_TYPE as $type) {
                foreach (Order::TRIP_COUNTRIES as $tripCountry) {
                    $cashback[$company][$tripCountry][$type] = GreencardCashback::where('insurance_company', $company)->where('trip_country', $tripCountry)->where('transport_type', $type)->orderBy('months')->pluck('amount', 'months');
                    $prices[$company][$tripCountry][$type] = GreenCardPrice::where('insurance_company', $company)->where('trip_country', $tripCountry)->where('transport_type', $type)->orderBy('months')->pluck('amount', 'months');
                }
            }
        }

        return view('greencard', compact('cashback', 'prices'));
    }
    ...
    public function updatePrices(GreenCardPriceRequest $request, string $company)
    {
        $data = $request->validated();
